...
{
    /// <summary>
    /// An implementation of IValueTaskCommand. Allows Commands to safely be used asynchronously with Task.
    /// </summary>
    public class ValueTaskCommand<T> : IValueTaskCommand<T>
    {
        readonly Func<T, ValueTask> _execute;
        readonly Func<object?, bool> _canExecute;
        readonly Action<Exception>? _onException;
        readonly bool _continueOnCapturedContext;
        readonly WeakEventManager.WeakEventManager _weakEventManager = new WeakEventManager.WeakEventManager();
        ...
        public ValueTaskCommand(Func<T, ValueTask> execute,
...
#pragma warning restore CS8601
    ...
    /// An implementation of IValueTaskCommand. Allows Commands to safely be used asynchronously with Task.
    /// </summary>
    public class ValueTaskCommand : IValueTaskCommand
    {
        ...
        /// <param name="onException">If an exception is thrown in the Task, <c>onException</c> will execute. If onException is null, the exception will be re-thrown</param>
        /// <param name="continueOnCapturedContext">If set to <c>true</c> continue on captured context; this will ensure that the Synchronization Context returns to the calling thread. If set to <c>false</c> continue on a different context; this will allow the Synchronization Context to continue on a different thread</param>
        public ValueTaskCommand(Func<ValueTask> execute,
                            Func<object?, bool>? canExecute = null,
                            Action<Exception>? onException = null,
                            bool continueOnCapturedContext = false)
        {
